<div class="card" 
x-data 
@click="$wire.readMsg(); $store.chatPanel.chatOn($wire.model);">
    <div class="card__img">
    ...
    <div class="card__content">
        <div class="card__content__head">
            <h1>{{ $model['title'] }}</h1>
            <small class="text-xs {{ !$model['status'] ? 'text-emerald-500':'text-zinc-500' }}">{{ $model['time'] }}</small>
        </div>
        <div class="card__content__body">
            <small class="text-xs {{ !$model['status'] ? 'text-white':'text-zinc-500' }}">{{ $model['excerpt'] }}</small>
